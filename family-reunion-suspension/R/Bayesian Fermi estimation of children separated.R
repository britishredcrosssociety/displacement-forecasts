...
simulate_fr <- function(
  ...
  if (is.null(rate_adjust)) {
    rate_adjust <- rep(1, horizon)
  }
...
p_children <- stats::rbeta(
  1000,
  shape1 = beta_prior$alpha,
  shape2 = beta_prior$beta
)
hist(p_children)
# Overlay actual data onto histogram
hist(
...
beta_prior_adults <- fit_beta_from_props(
  fr_adults_kids[fr_adults_kids$age_group == "Adult", ]$prop,
  totals = fr_summary$visas_granted
) # uses reconstructed successes/failures
p_adults <- stats::rbeta(
  1000,
  shape1 = beta_prior_adults$alpha,
  shape2 = beta_prior_adults$beta
)
hist(p_adults)
# Overlay actual data onto histogram
hist(
