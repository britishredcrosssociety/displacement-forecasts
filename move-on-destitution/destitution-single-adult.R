...
single_adult_destitute <- all_nationalities |>
  ...
  select(1, 14, 15, 16) |>
  mutate(destitute_total = if_else(destitute_total == 0, NA_real_, destitute_total),
         destitute_total_upper = if_else(destitute_total_upper == 0, NA_real_, destitute_total_upper),
         destitute_total_lower = if_else(destitute_total_lower == 0, NA_real_, destitute_total_lower)) |>
...
ggplot(single_adult_destitute, aes(x = quarter, group = 1)) +
  ...
  geom_line(aes(y = destitute_total, color = "Estimated historic risk"), size = 1) +
  geom_point(aes(y = destitute_total, color = "Estimated historic risk"), size = 2) +
  
  # Ribbon with projection range
  geom_ribbon(aes(ymin = destitute_total_lower, ymax = destitute_total_upper, fill = "Projection range"), alpha = 0.3) +
  
  # Dashed upper/lower bounds
  geom_line(aes(y = destitute_total_upper), linetype = "dashed", color = "black", show.legend = FALSE) +
  geom_line(aes(y = destitute_total_lower), linetype = "dashed", color = "black", show.legend = FALSE) +
  ...
  # Labels
  labs(
    title = "Quarterly Number of Newly Granted Single Adult \nRefugees At-Risk of Destitution, United Kingdom",
    x = "",
    y = "Number of people",
...
ggsave("move-on-destitution/projection.png")

# ---- Sense check ----
# Aggregated nationalities and filtered all nationalities roughly the same?
single_adult <- read_csv("move-on-destitution/granted-status-data/positive_projected_single_adult_all nationalities.csv") |>
  select(1,2,3,4) |> slice(-22:-23)

single_adult_agg <- all_nationalities |> slice(-22:-23)

compare <- tibble(date = single_adult_agg$date,
                  single_adult_difference_perc = (single_adult$total_decisions - single_adult_agg$total_decisions) / 
                    single_adult_agg$total_decisions * 100) 

# Num of single adult < all people?
# need to regenerate this link each time as its a private repo
all_people <- read_csv("https://raw.githubusercontent.com/britishredcrosssociety/rs-destitution/refs/heads/main/data/positive_projected_all%20nationalities.csv?token=GHSAT0AAAAAADGEAMOIQ7UQCSVT7SNWQLM42GCWNUA") |>
  select(1,2,3,4) |>
  slice(-22:-23)

compare <- compare |>
  mutate(all_people_single_adult_diff = all_people$total_decisions - single_adult_agg$total_decisions)
